...
public class GameManager : MonoBehaviour {
	...
	private void SpawnLocalPlayer(int numModel, int numMaterial) {
		...
		localPlayer.AddComponent<RigidbodyFirstPersonController>();
		localPlayerController = localPlayer.GetComponent<RigidbodyFirstPersonController>();
		localPlayer.GetComponent<Rigidbody>().mass = 10;
		localPlayer.GetComponent<CapsuleCollider>().material = (PhysicMaterial)Resources.Load("PhysicMaterials/ZeroFriction");
		localPlayer.GetComponentInChildren<TextMesh>().text = smartFox.MySelf.Name;
		//Camera.main.transform = new Vector3(0,0,0)
		Camera.main.transform.parent = localPlayer.transform;
		localPlayerController.cam = Camera.main;
